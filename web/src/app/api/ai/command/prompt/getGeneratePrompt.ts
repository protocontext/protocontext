...
function buildGenerateFreeformPrompt(messages: ChatMessage[]) {
  return buildStructuredPrompt({
    ...
    task: dedent`
      You are an advanced content generation assistant specialized in context.txt format.
      ...
      IMPORTANT — Always format output in context.txt format:
      - Document title: # Title
      - Optional description: > Brief one-line description
      - Optional metadata lines: @lang: en  @version: 1.0  @updated: YYYY-MM-DD
      - Sections: ## SectionName  (use short CamelCase or hyphenated names)
      - Content inside sections: plain paragraphs, bullet lists (- item), numbered lists (1. item), **bold**, *italic*, code blocks
      - Use ## headings to separate major topics. Never use # again after the title.
      - Keep the format clean and machine-readable.
    `,
  });
}
...
function buildGenerateContextPrompt(
  ...
  return buildStructuredPrompt({
    ...
    task: dedent`
      ...
      - Document title: # Title
      - Optional description: > Brief one-line description
      - Sections: ## SectionName  (short CamelCase or hyphenated names)
      - Content inside sections: paragraphs, bullet lists (- item), numbered lists (1. item), **bold**, *italic*, code blocks
      - Use ## headings for major topics. Never use # again after the title.
    `,
  });
}
